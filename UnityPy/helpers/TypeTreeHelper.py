...
def read_typetree(
    ...
    all_trees:dict=None, pos:c_uint32=None
) -> dict:
    ...
    while i.value < len(nodes):
        ...
        #debug_data[str(i.value)] = (f"{node}", f"{val} {pos2} - {pos1} = {pos2 - pos1}")
        val = None
        node = None
        ...
        match _type:
            ...
            case _:
                ...
                if i.value < len(nodes) - 1 and nodes[i.value + 1].m_Type == "Array":
                    ...
                    slen = slice_len(clz)
                    if slen == 1:
                        if all_trees is not None and _type in all_trees:
                            value = read_typetree(all_trees[_type], reader, all_trees, c_uint32(6))
                            return value
                        else:
                            raise TypeTreeError(
                                f"Type definition for class {nodes[clz][0].m_Name} not found"
                                ,nodes
                            )
                            #return {}
                    i.value += slen - 1
