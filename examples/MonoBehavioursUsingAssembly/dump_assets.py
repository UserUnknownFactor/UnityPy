...
TYPES = ['MonoBehaviour', 'PlayerSettings', 'TextAsset', 'Texture2D']#, 'Sprite', 'Shader']
...
def export_obj(obj, asset: str, local_path: str) -> list:
    ...
    if objfmt == "TextAsset":
        if data.script:
            fp = f"{make_path(DST, 'TextAsset', local_path, os.path.split(fname)[0], objname)}.txt"
            if not os.path.isfile(fp):
                with open(fp, "wb") as f:
                    f.write(data.script)

    elif objfmt == "Texture2D":
        fp = f"{make_path(DST, 'Texture2D', local_path, fname)}.png"
        if not os.path.isfile(fp):
            try:
                data.image.save(fp)
        ...
        fp = f"{make_path(DST, 'Sprite', local_path, fname)}.png"
        if not os.path.isfile(fp):
            data.image.save(fp)

    elif objfmt == "PlayerSettings":
        #data.save()
        fp = f"{make_path(DST, 'PlayerSettings', local_path, objname)}.dat"
        if not os.path.isfile(fp):
            with open(fp, "wb") as f:
                f.write(obj.data)
        ...
        if script:
            fp = f"{make_path(DST, 'MonoBehaviour', local_path, script.m_Namespace, script.m_ClassName, objname)}.{extension}"
        else:
            fp = f"{make_path(DST, 'MonoBehaviour', local_path, objname)}.{extension}"
        if not os.path.isfile(fp):
            with open(fp, "wb") as f:
                f.write(export)
